...
static uint8_t individual_channel_stream(NeAACDecHandle hDecoder, element *ele,
                                         ...
                                         int16_t *spec_data, uint8_t channel);
                                          ...
                                          uint8_t channel, uint8_t *tag)
{
    ...
    *tag = sce.element_instance_tag;
    ...
    retval = individual_channel_stream(hDecoder, &sce, ld, ics, 0, spec_data, 0);
    if (retval > 0)
        return retval;
                               ...
                               bitfile *ld, program_config *pce, drc_info *drc)
{
    ...
    for (ch = 0; ch < (this_layer_stereo ? 2 : 1); ch++)
    {
        ...
        hInfo->error = individual_channel_stream(hDecoder, &cpe, ld, ics, 1, spec_data, ch);
        if (hInfo->error > 0)
            return;
    }
